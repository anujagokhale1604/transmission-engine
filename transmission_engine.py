...
def run_granger(df, maxlag=4):
    pairs = [
        ("India","Singapore",True),("India","UK",True),("Singapore","UK",True),
    ...
    for cause, effect, expected in pairs:
        ...
        results.append({"cause":cause,"effect":effect,"pval":best_pval,
                        "lag":best_lag,"significant":best_pval<0.05,"expected":expected})
    return results
...
with t1:
    ...
    fig.add_annotation(x="2022-04-01", y=1.04, yref="paper",
    ...
    with col_out:
        ...
        for col_idx, country in enumerate(["Singapore","India"]):
            ...
            if india_shock != 0:
                ...
                fig3.add_trace(go.Scatter(
                    x=fc_shock.index.strftime("%Y-%m-%d"), y=fc_shock[country],
    ...
    try:
        ...
        for resp_idx, resp_country in enumerate(countries):
            fig_irf.add_trace(go.Scatter(
                x=list(range(13)), y=irf_vals[:,resp_idx,shock_idx],
